...
# Gradient Loss channal=1
class Gradient_Loss_chn1(nn.Module):
    def __init__(self, losstype='l2'):
        super(Gradient_Loss_chn1, self).__init__()
        # a = np.array([1, 0, -1])
        a = np.array([[1, 0, -1], [2, 0, -2], [1, 0, -1]])
        conv1 = nn.Conv2d(1, 1, kernel_size=3, stride=1, padding=1, bias=False, groups=1) #mcj in_channal:3 1
        a = torch.from_numpy(a).float().unsqueeze(0).unsqueeze(0)
        # a = torch.stack((a, a, a))
        ...
        conv2 = nn.Conv2d(1, 1, kernel_size=3, stride=1, padding=1, bias=False, groups=1) #mcj in_channal:3 1
        b = torch.from_numpy(b).float().unsqueeze(0).unsqueeze(0)
        # b = torch.stack((b, b, b))
...
class Gradient_Loss(nn.Module):
    def __init__(self, losstype='l2'):
        ...
        a = np.array([[1, 0, -1], [2, 0, -2], [1, 0, -1]])
        conv1 = nn.Conv2d(3, 3, kernel_size=3, stride=1, padding=1, bias=False, groups=3)  # mcj in_channal:3 1
