...
from models.other_networks.DnCNN import DnCNN
...
class DnCNN_model_aug(BaseModel):
    def __init__(self, opt):
        super(DnCNN_model_aug, self).__init__(opt)
        ...
        self.net = DnCNN(in_channels=1, out_channels=1, dep=17, num_filters=64, slope=0.2).to(self.device)
        ...
        if self.is_train:
            ...
            # loss
            self.MSE_loss = ReconstructionLoss(losstype='l2')#nn.MSELoss(reduce = True,size_average = False)
    ...
    def save(self, iter_label):
        self.save_network(self.net, 'DnCNN', iter_label)
