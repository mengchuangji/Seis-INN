...
class InvDN_Model_C(BaseModel):
    def __init__(self, opt):
        super(InvDN_Model_C, self).__init__(opt)
        ...
        if self.is_train:
            ...
            self.Rec_Forw_grad = Gradient_Loss_chn1() #mcj Gradient_Loss()
            self.Rec_back_grad = Gradient_Loss_chn1() #mcj Gradient_Loss()
    ...
    def feed_data_aug(self, data):
        ...
        with torch.autograd.no_grad():
            ...
            # self.noisy_H[NReal:, ].clamp_(-1.0, 1.0)
    ...
    def loss_backward(self, x, y):
        ...
        x_samples_image = x_samples[:, :self.s_c, :, :] # c=3 or c=1
        ...
        return l_back_rec + self.train_opt['lambda_SSIM_Grad']*(l_grad_back_rec + l_back_SSIM)
    ...
    def optimize_parameters(self, step):
        ...
        # C= self.noisy_H.shape[1]
        LR_ref = self.ref_L.detach()

        l_forw_ce = 0
        l_forw_fit = self.loss_forward(self.output[:, :self.s_c, :, :], LR_ref)

        # backward
        gaussian_scale = self.train_opt['gaussian_scale'] if self.train_opt['gaussian_scale'] != None else 1
        y_ = torch.cat((self.output[:, :self.s_c, :, :], gaussian_scale * self.gaussian_batch(self.output[:, self.s_c:, :, :].shape)), dim=1)
    ...
    def generateFakeNoisy(self,epsilon):
        ...
        self.FakeNoisy = self.netG(x=y_forw, rev=True)#[:, :self.s_c, :, :]
    ...
    def test(self, self_ensemble=False):
        ...
        with torch.no_grad():
            if self_ensemble:
                ...
                output = self.netG(x=self.input)
                # C=self.input.shape[1]
                self.forw_L = output[:, :self.s_c, :, :]
                y_forw = torch.cat((output[:, :self.s_c, :, :], gaussian_scale * self.gaussian_batch(output[:, self.s_c:, :, :].shape)), dim=1)
                self.fake_H = self.netG(x=y_forw, rev=True)[:, :self.s_c, :, :]
        ...
        with torch.no_grad():
            if self_ensemble:
                ...
                for i in range(sample_num):
                    y_forw = torch.cat((output[:, :self.s_c, :, :], gaussian_scale * self.gaussian_batch(output[:, self.s_c:, :, :].shape)), dim=1)
                    fake_Hs.append(self.netG(x=y_forw, rev=True)[:, :self.s_c, :, :])
    ...
    def forward_x8(self, x, forward_function, gaussian_scale):
        ...
        back_list = []
        for data in lr_list:
            y_forw = torch.cat((data[:, :self.s_c, :, :], gaussian_scale * self.gaussian_batch(data[:, self.s_c:, :, :].shape)), dim=1)
