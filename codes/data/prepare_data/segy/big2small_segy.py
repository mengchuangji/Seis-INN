...
def generate_patch_from_segy(dir,pch_size,stride):
    ...
    for i in range(len(file_list)):
        ...
        data = np.asarray([np.copy(x) for x in f.trace[:]]).T[:,:15000] #[:,130260:] #[:,:19500]
        print(data.max())
        print(data.min())
    ...
    for i in range(len(file_list)):
        ...
        for j in range(0, shot_num, jump):
            ...
            if agc:
                ...
                # data = data / abs(data).max() #合成实验
                data = data / abs(data).max() #实际数据实验
...
def generate_patch_from_poststack_segy_1by1(dir,pch_size,stride,jump=1,agc=False,train_data_num=float('inf'),trace_num=20000,section_num=40,aug_times=[],scales = []):
    '''
    Args:
        aug_time(list): Corresponding function data_aug, if aug_time=[],mean don`t use the aug
        scales(list): data scaling; default scales = [],mean that the data don`t perform scaling,
    ...
    for i in range(len(file_list)):
        ...
        print('正在处理第'+str(i+1)+'个数据集...')
        trace_num = len(sourceX)  # number of trace, The sourceX under the same shot is the same character.
        section_num = section_num # shot number
        len_section = trace_num//section_num  # The length of the data in each shot data
        for j in range(0, int(section_num*0.25), jump): #*0.75
            data = np.asarray([np.copy(x) for x in f.trace[j * len_section:(j + 1) * len_section]]).T
            # data = np.asarray([np.copy(x) for x in f.trace[0:5000]]).T # mcj 剖面分割
            if agc:
                data = gain(data, 0.004, 'agc', 0.05, 1)
            else:
                data = data #/ abs(data).max()  #成对数据要考虑整体归一化
            # 不用 scales 及 data augmentation
            H = data.shape[0]
            W = data.shape[1]
            ind_H = list(range(0, H - pch_size[0] + 1, stride[0]))
    ...
    print('Finish!\n')
    patches=np.array(patches)
    return patches[:, :, :, np.newaxis] #  patches[:, :, :]
