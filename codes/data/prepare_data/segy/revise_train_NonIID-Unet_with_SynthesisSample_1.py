...
from networks import weight_init_kaiming
...
args = set_opts()
args.log_dir= './log_test1'
args.model_dir='./model_test1'
args.gpu_id=2
...
def main():
    ...
    args.siesmic_dir = './datasets/seismic/marmousi'
    from datasets.prepare_data.mat.bia2small_mat import  generate_patch_from_mat
    train_im_list = generate_patch_from_mat(dir="./datasets/seismic/marmousi", pch_size=64, stride=[64, 64])

    # making othrt systhesis training data
    ...
    train_field_list = datagenerator(data_dir=args.data_dir, patch_size=(64,64), stride=args.stride,
    ...
    # train_field_list=train_field_list/abs(train_field_list).max
    train_im_list=np.concatenate([train_im_list, train_field_list], axis=0)
    train_im_list = train_im_list.astype(np.float32)

    test_im_list = generate_patch_from_mat(dir="./datasets/seismic/overthrust", pch_size=128,
                                           stride=[24, 64])
    test_im_list = test_im_list.astype(np.float32)


    test_case1_h5 = Path('./datasets/seismic/overthrust').joinpath('noise_niid',
                                                                                  'overthrust_niid_case1.hdf5')
    test_case2_h5 = Path('./datasets/seismic/overthrust').joinpath('noise_niid',
                                                                                  'overthrust_niid_case2.hdf5')
    test_case3_h5 = Path('./datasets/seismic/overthrust').joinpath('noise_niid',
                                                                                  'overthrust_niid_case3.hdf5')
    test_case4_h5 = Path('./datasets/seismic/overthrust').joinpath('noise_niid',
                                                                                  'overthrust_niid_case4.hdf5')
    test_case5_h5 = Path('./datasets/seismic/overthrust').joinpath('noise_niid',
                                                                                  'overthrust_niid_case5.hdf5')
    # sigma = 0.001 * 255
    args.patch_size=(32,32)
    datasets = {'train':     DenoisingDatasets_seismic.SimulateTrain_sigma(train_im_list, 200000,
                                                                           args.patch_size, radius=args.radius,
                                                                           noise_estimate=True, sigma=75),
