...
#    all_patches = np.delete(all_patches, range(discard_n), axis=0)
    ...
    return all_patches
def datagenerator_tar(data_dir,patch_size = (128,128),stride = (32,32), train_data_num = float('inf'),download=True,datasets = "Hess_VTI",aug_times=0,scales = [1],verbose=True,jump=1,agc=True):
    '''
    Args:
        data_dir : the path of the .segy file exit
        patch_size : the size the of patch
    ...
    for i in range(len(file_list)):
        with segyio.open(file_list[i],'r',ignore_geometry=True) as f:
            ...
            for j in range(0,shot_num,jump):
                # data = np.asarray([np.copy(x) for x in f.trace[j*len_shot:(j+1)*len_shot]]).T
                data = np.asarray([np.copy(x) for x in f.trace[0:2500]]).T # mcj 剖面分割
                q += 1
                if agc:
                    data = gain(data,0.004,'agc',0.05,1)
                else:
...
#    all_patches = np.delete(all_patches, range(discard_n), axis=0)
    ...
    return all_patches
def datagenerator_test(data_dir,patch_size = (128,128),stride = (32,32), train_data_num = float('inf'),download=True,datasets = "Hess_VTI",aug_times=0,scales = [1],verbose=True,jump=1,agc=True):
    '''
    Args:
        data_dir : the path of the .segy file exit
        patch_size : the size the of patch
    ...
    for i in range(len(file_list)):
        with segyio.open(file_list[i],'r',ignore_geometry=True) as f:
            ...
            for j in range(0,shot_num,jump):
                # data = np.asarray([np.copy(x) for x in f.trace[j*len_shot:(j+1)*len_shot]]).T
                data = np.asarray([np.copy(x) for x in f.trace[15000:20000]]).T
                q += 1
                if agc:
                    data = gain(data,0.004,'agc',0.05,1)
                else:
